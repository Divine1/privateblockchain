...
class Blockchain{
   ...
   async genesisBlock(genesisBlock){
    ...
    async addBlock(newBlock){
        ...
        if(chainLength >0){
             let previousBlock = await getLevelDBData(chainLength - 1);
             newBlock.previousBlockhash = previousBlock.hash;
        }
        newBlock.hash = SHA256(JSON.stringify(newBlock)).toString();
        console.log("newBlock ",newBlock)
        addLevelDBData(chainLength,JSON.stringify(newBlock))
    }
    ...
    // get block
     async getBlock(blockHeight){
      // return object as a single string
        let block = await getBlockUsingHeight(blockHeight);
    ...
    async validateChain(){
      ...
      for (var i = 0; i < chainLength-1; i++) {
        ...
        let chain_i = await getLevelDBData(i);
        console.log("chain_i ",chain_i," == ",i)
        let blockHash = chain_i.hash;
        let chain_ip1 = await getLevelDBData(i+1);
